...
@app.event("http")
def exchange_code(req: BoltRequest, resp: BoltResponse):
    data = req.body
    code = data.get("code")
    ...
    try:
        ...
        resp.status = 500
        resp.body = {
            "error": "Failed to retrieve the access token from Slack.",
    ...
    if not slack_data.get("ok", False):
        ...
        resp.status = 500
        resp.body = {
            "error": f"Slack API returned an error: {error_message}",
            "response_type": "ephemeral",
        }
        return
    ...
    if not access_token:
        resp.status = 500
        resp.body = {"error": "Access token not found in Slack response."}
        return
    ...
    if not user_id or not scope or not token_type:
        resp.status = 500
        resp.body = {"error": "Missing data in authed_user field."}
        return
    ...
    if not team_id:
        resp.status = 500
        resp.body = {"error": "Missing team id in Slack response."}
        return
    ...
    if count == 0:
        resp.status = 500
        resp.body = {
            "error": "Slack Access Code was not successfully saved. Email support@getcleo.io with this error in subject line."
        }
        return
